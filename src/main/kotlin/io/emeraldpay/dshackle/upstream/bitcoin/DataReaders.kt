...
open class DataReaders(
    ...
    init {
        blockReader = object : Reader<BlockId, BlockContainer> {
            override fun read(key: BlockId): Mono<BlockContainer> {
                return getBlock(key, 1)
            }
        }

        txReader = object : Reader<TxId, TxContainer> {
            override fun read(key: TxId): Mono<TxContainer> {
                return readBytes(DshackleRequest("getrawtransaction", listOf(key.toHex(), true)))
    ...
    open fun getTx(txId: TxId): Mono<TxContainer> {
        return txReader.read(txId)
    }

    open fun getBlockJson(hash: BlockId): Mono<Map<String, Any>> {
        return getBlock(hash, 1)
            .map { it.parsed as Map<String, Any> }
    }

    open fun getBlockJson(height: Long): Mono<Map<String, Any>> {
        return readJson(DshackleRequest("getblockhash", listOf(height)), String::class.java)
    ...
    fun readBytes(req: DshackleRequest): Mono<ByteArray> {
        return source.read(req)
            .flatMap(DshackleResponse::requireResult)
    }

    fun <T> readJson(req: DshackleRequest, clazz: Class<T>): Mono<T> {
        return source.read(req)
            .flatMap(DshackleResponse::requireResult)
            .map {
